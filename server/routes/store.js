...
const storage = multer.diskStorage({
    destination: (req, file, cb) => {
        cb(null, 'stores/'); // Define destination folder for uploaded images
    },
...
router.post('/upload-store', upload.single('image'), async (req, res) => {
    try {
        const imageUrl = `${req.protocol}://${req.get('host')}/stores/${req.file.filename}`;
...
router.get('/image-store', (req, res) => {
    const uploadDirectory = path.join(__dirname, '../stores');
    ...
    try {
        ...
        fs.readdir(uploadDirectory, (err, files) => {
            ...
            const imageUrls = files.map((file) => {
                return `${req.protocol}://${req.get('host')}/stores/${file}`;
            });
...
router.delete('/delete-store', (req, res) => {
    ...
    const imagePath = path.join(__dirname, '../stores/', getImageFilenameFromUrl(imageUrl));
...
router.post('/replace-store', upload.single('image'), (req, res) => {
    try {
        ...
        const newImageUrl = `/stores/${newImageFilename}`;
        ...
        if (previousImageUrl) {
            const previousImagePath = path.join(__dirname, '../stores/', getImageFilenameFromUrl(previousImageUrl));
