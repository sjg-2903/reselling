...
const storage = multer.diskStorage({
    destination: (req, file, cb) => {
        cb(null, 'brands/'); // Define destination folder for uploaded images
    },
...
router.post('/upload-brand', upload.single('image'), async (req, res) => {
    try {
        const imageUrl = `${req.protocol}://${req.get('host')}/brands/${req.file.filename}`;
...
router.get('/image-brand', (req, res) => {
    const uploadDirectory = path.join(__dirname, '../brands');
    ...
    try {
        ...
        fs.readdir(uploadDirectory, (err, files) => {
            ...
            const imageUrls = files.map((file) => {
                return `${req.protocol}://${req.get('host')}/brands/${file}`;
            });
...
router.delete('/delete-brand', (req, res) => {
    ...
    const imagePath = path.join(__dirname, '../brands/', getImageFilenameFromUrl(imageUrl));
...
router.post('/replace-brand', upload.single('image'), (req, res) => {
    try {
        ...
        const newImageUrl = `/brands/${newImageFilename}`;
        ...
        if (previousImageUrl) {
            const previousImagePath = path.join(__dirname, '../brands/', getImageFilenameFromUrl(previousImageUrl));
